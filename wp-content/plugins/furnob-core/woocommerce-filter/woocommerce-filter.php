...
/* Min/Max Quantity */
if(get_theme_mod('furnob_min_max_quantity',0) == 1){
...
/* Min Amount */
if(get_theme_mod('furnob_min_order_amount_toggle',0) == 1){
	require_once( __DIR__ . '/min-amount/min-amount.php' );
}

/* Buy Now Single */
if(get_theme_mod('furnob_shop_single_buy_now',0) == 1 || furnob_ft() == 'buynow'){
...
/* Registration Fields */
if(get_theme_mod('furnob_registration_first_name') == 'visible' || get_theme_mod('furnob_registration_last_name') == 'visible' || get_theme_mod('furnob_registration_billing_company') == 'visible' || get_theme_mod('furnob_registration_billing_phone') == 'visible'){
...
if(get_theme_mod('furnob_single_sticky_titles',0) == 1){
	require_once( __DIR__ . '/single-sticky-titles/single-sticky-titles.php' );
}

/* Remove All button */
if(get_theme_mod('furnob_remove_all_button',0) == 1){
	require_once( __DIR__ . '/remove-all/remove-all.php' );
}

/* Ajax Login */
if(get_theme_mod('furnob_ajax_login_form',0) == 1){
	require_once( __DIR__ . '/ajax-login/ajax-login.php' );
}

/* Sticky Single Cart */
if(get_theme_mod('furnob_single_sticky_cart',0) == 1 || furnob_ft() == 'stickycart'){
	require_once( __DIR__ . '/sticky-single-cart/sticky-single-cart.php' );
}

/* Ajax Search */
if(get_theme_mod('furnob_ajax_search_form',0) == 1){
...
## Order on WhatsApp
*************************************************/
if(get_theme_mod('furnob_shop_single_orderonwhatsapp', 0) == 1 || furnob_ft() == 'orderonwhatsapp'){
	add_action('woocommerce_single_product_summary', 'furnob_order_on_whatsapp', 45);
	function furnob_order_on_whatsapp(){
		$number = get_theme_mod('furnob_shop_single_whatsapp_number');
		
		echo '<div class="orderon-whatsapp">';
		echo '<a href="https://wa.me/'.esc_attr($number).'?text='.get_the_title().'" class="button button-primary xsmall rounded">'.esc_html__('Order on WhatsApp','furnob-core').'</a>';
		echo '</div>';
	}
}
...
*************************************************/
if(get_theme_mod('furnob_shop_single_review_tab_move', 0) == 1 || furnob_ft() == 'movereviewtab'){
	add_action( 'woocommerce_after_single_product_summary', 'furnob_move_review_tab', 5 );
	function furnob_move_review_tab(){
	  comments_template();
	}

	add_filter( 'woocommerce_product_tabs', 'furnob_remove_product_tabs', 98 );
	function furnob_remove_product_tabs( $tabs ){
		unset( $tabs['reviews'] );
...
*************************************************/
add_filter('woocommerce_login_redirect', 'furnob_myaccount_redirect_after_login');
function furnob_myaccount_redirect_after_login($redirect_to) {
	if(get_theme_mod('furnob_redirect_url_after_login')){
		return esc_url(get_theme_mod('furnob_redirect_url_after_login'));
	} else {
...
*************************************************/
function furnob_product_stock_progress_bar(){
	global $product;
	...
	if($managestock) {
		...
		$output .= '<div class="sold">'.esc_html__('Sold:','furnob-core').' <strong>'.esc_html($total_sales).'</strong></div>';
		$output .= '<div class="available">'.esc_html__('Available:','furnob-core').' <strong>'.esc_html($stock_quantity).'</strong></div>';
		$output .= '</div>';
...
*************************************************/
function furnob_product_time_countdown(){		
	$id = get_the_ID();
	...
	if($sale_price_dates_to){
		wp_enqueue_script( 'furnob-counter');
		...
		$output .= '<div class="expired-text">'.esc_html__('Remains until the end of the offer','furnob-core').'</div>';
